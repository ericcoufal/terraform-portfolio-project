...
Files are stored in a **private S3 bucket** and serverd globally over HTTPS through **CloudFront** (AWS's CDN). The bucket is never public. CloudFront reaches it through **Origin Access Control (OAC)** with SigV4 signing, and a bucket policy grant `s3:GetObject` to *only this specific distribution* - one controlled entry point instead of two.

# Why private S3 + OAC

The common tutorial pattern makes the bucket public. That leaves a second front door: 
anyone who learns the bucket address bypasses the CDN entirely. Keeping the bucket private and giving CloudFront its own identity closes that door. Trade-off: ~20% more Terraform
(OAC resource, scoped bucket policy, four public-access blocks) in exchange for a single entry point and least-privilege access.
...
- An AWS account with credentials configures locally (via aws configure`, environment variables, or an SSO profile)
...
> **Note:** The S3 bucket name is globally unique - `ericcoufal-portfolio-site`is taken.
> Change the `bucket`value in `main.tf`to something unique before deploying your own copy.
